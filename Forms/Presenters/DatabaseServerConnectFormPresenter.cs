...
namespace Pflegehaushaltsbuch.Forms.Presenters
{
    public class DatabaseServerConnectFormPresenter
    {
        ...
        public DatabaseServerConnectFormPresenter(IDatabaseServerConnectFormContract view, SqlSession session, XmlConfig config)
        {
            if (view == null)
            {
        ...
        {
            SQLBase sql = CreateSqlProviderOrNull(config.DBType);
            if (sql == null)
            {
        ...
        private static SQLBase CreateSqlProviderOrNull(XmlConfig.DataBaseTypes dbType)
        {
