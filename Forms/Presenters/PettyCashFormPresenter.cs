...
{
    public class PettyCashFormPresenter
    {
        ...
        public PettyCashFormPresenter(IPettyCashFormContract view, SqlSession session)
        {
            if (view == null)
            {
        ...
        protected IPettyCashFormContract View { get; private set; }
        ...
        public virtual void ApplyUserRights(UserRights rights)
        {
            ...
            View.SetGridUpdateVisible(false);
            View.SetBookButtonsEnabled(rights.CanAccessPettyCash && rights.CanBook, rights.CanAccessPettyCash && rights.CanCancelBooking);
        }
        ...
        public virtual async Task ConnectTableToDataBaseAsync()
        {
            ...
            View.SetTotalAmount(Convert.ToDecimal(officeTotalAmount).ToString("C", session.Company.Currencies));

            table = new DataTable();
            if (!View.PeriodChecked)
            {
                DateTime date = View.FromDate;
                await session.SQL.FillAdapterAsync(SQLBase.SELECT.PettyCashByDate, table, date.Month, date.Year);
            }
            else
            {
        ...
        private async Task<bool> InitializePeriodDateRangeAsync()
        {
            ...
            await session.SQL.FillAdapterAsync(SQLBase.SELECT.PettyCash, allBookings);
        ...
        public virtual async Task PrintAsync()
        {
            ...
            await session.SQL.FillAdapterAsync(SQLBase.SELECT.PettyCash, fullOfficeCash);
            ...
            {
                previousAmount += Convert.ToDecimal(row["amount"]);
            }
            ...
            foreach (DataRow row in rows)
            {
                ...
                decimal value = Math.Abs(Convert.ToDecimal(row["amount"]));
                if (category == 0)
                    einnahmen += value;
                else
                    ausgaben += value;
            ...
            session.SQL.Printing.UpdateVariable(Data.Printing.VarNames.amount_previous_month, previousAmount.ToString("C", session.Company.Currencies));
            session.SQL.Printing.UpdateVariable(Data.Printing.VarNames.amount, (previousAmount - ausgaben + einnahmen).ToString("C", session.Company.Currencies));
