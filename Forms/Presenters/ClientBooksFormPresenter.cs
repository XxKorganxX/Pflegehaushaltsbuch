...
{
    public class ClientBooksFormPresenter
    {
        ...
        public ClientBooksFormPresenter(IClientBooksFormContract view, SqlSession session)
        {
            if (view == null)
            {
        ...
        protected IClientBooksFormContract View { get; private set; }

        public virtual void SetClientID(int clientID)
        {
            if (this.clientID != clientID)
                periodDateRangeInitialized = false;
        ...
        private void InitializePeriodDateRange()
        {
            if (periodDateRangeInitialized || clientTable.Rows.Count == 0)
                return;

            DataRow clientRow = clientTable.Rows[0];
            object dateValue = clientRow[Columns.Date];
            if (dateValue == DBNull.Value)
                return;

            DateTime fromDate = Convert.ToDateTime(dateValue);
            DateTime toDate = DateTime.Now;
            if (clientTable.Columns.Contains(Columns.LastBook) && clientRow[Columns.LastBook] != DBNull.Value)
                toDate = Convert.ToDateTime(clientRow[Columns.LastBook]);

            if (toDate < fromDate)
                toDate = fromDate;

            View.SetPeriodDateRange(fromDate, toDate);
        ...
        public virtual async Task StornoAsync(DataRow row)
        {
            ...
            try
            {
                ...
                decimal amount = Convert.ToDecimal(row[Columns.Amount]);
                ...
                using (var transaction = session.SQL.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (bookingTo == SQLBase.BookingTo.Bankbestand)
                            stornoBooked = await session.SQL.ToBankAsync(DateTime.Parse(row["date"].ToString()), row["note"].ToString(), -amount, accountId, SQLBase.BookCategory.Storno, SQLBase.BookingTo.Bankbestand);
                        else if (bookingTo == SQLBase.BookingTo.Barbestand)
                            stornoBooked = await session.SQL.ToBargeAsync(DateTime.Parse(row["date"].ToString()), row["note"].ToString(), -amount, accountId, SQLBase.BookCategory.Storno, SQLBase.BookingTo.Barbestand);
                        ...
                        try
                        {
                            ...
                            else if (bookTo == SQLBase.BookingTo.Bankbestand)
                            {
                                if (bookCategory == SQLBase.BookCategory.Einzahlung)
                                {
                                    ...
                                    currentBook = result.Item2;
                                    if (valid = result.Item1)
                                        valid = await session.SQL.ToBankAsync(payInDate.Date.Date, bookText, amount, accountId, SQLBase.BookCategory.Einzahlung, bookTo);
                                }
                                else
                                {
                                    var result = await session.SQL.ToBooksAsync(clientName, clientId, payInDate.Date.Date, bookText, -amount, SQLBase.BookCategory.Auszahlung, bookTo);
                                    currentBook = result.Item2;
                                    if (valid = result.Item1)
                                        valid = await session.SQL.ToBankAsync(payInDate.Date.Date, bookText, -amount, accountId, SQLBase.BookCategory.Auszahlung, bookTo);
                                }
                            }
                            if (!valid)
