...
namespace Pflegehaushaltsbuch.Tests
{
    ...
    public class DatabaseFunctionTests
    {
        ...
        public async Task UpdateAsistanceAsyncUpdatesRepaymentAndClosesPaidLoan()
        {
            ...
            Assert.IsTrue(saved);
            DataRow row = sql.Table(SQLBase.SELECT.Emploees).Rows[0];
        ...
        public async Task TransactionCommitKeepsAllTableChanges()
        {
            ...
            Assert.AreEqual(1, sql.Table(SQLBase.SELECT.Cash).Rows.Count);
        }
        ...
        {
            FakeSqlDatabase sql = new FakeSqlDatabase();
            sql.FailUpdatesFor(SQLBase.SELECT.Cash);
            ...
            Assert.AreEqual(0, sql.Table(SQLBase.SELECT.Cash).Rows.Count);
        }
        ...
        private sealed class FakeSqlDatabase : SQLBase
        {
            ...
            {
                DataRow row = tables[SELECT.Emploees].NewRow();
                row["id"] = id;
                row["name"] = name;
                ...
                row["active"] = active;
                row["handsign"] = "Initial";
                tables[SELECT.Emploees].Rows.Add(row);
                tables[SELECT.Emploees].AcceptChanges();
            }
