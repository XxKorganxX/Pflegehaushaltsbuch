            ...
            for (; dataIndex < data.Length; )
            {
                ...
                foreach (DataRow row in TableDesign.Rows)
                {
                    ...
                    */
                    ...
                    else if (id.Equals(SQLBase.Names(SQLBase.ColumnNames.book_cat)))
                        text = ((SQLBase.BookCategory)Int32.Parse(text)).GetDisplayName();
                    else if (id.Equals(SQLBase.Names(SQLBase.ColumnNames.id)))
                        text = string.Format("{0:000}", Int32.Parse(text));
                    else if (id.Equals(SQLBase.Names(SQLBase.ColumnNames.active)))
                    {
                        //Alte Version 
                        text = ((SQLBase.ClientActive)Int32.Parse(text)).GetDisplayName();
                    }
                    else if (id.Equals(SQLBase.Names(SQLBase.ColumnNames.amount_payback_type)))
