...
namespace Pflegehaushaltsbuch.Data.Print
{
    ...
    public class PrintBase
    {
        ...
        /// </summary>
        public PrintBase(SqlSession session, Printing.LayoutEnum layer)
        {
            Session = session;
            this.layer = layer;
            document = session.SQL.Printing.Layouts[layer];
        }
        /// <summary>
        ...
        protected virtual void OnPrint_Pdf_Page(PdfDocument pdfDocument)
        {
            ...
            DocumentLayer document = Session.SQL.Printing.Layouts[layer];
            List<GraphicsItem> items = document.Items;
            foreach (var item in items)
                item.Encrypt(Session.SQL);
            foreach (var item in items)
