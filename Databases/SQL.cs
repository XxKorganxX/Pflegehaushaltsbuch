...
namespace Pflegehaushaltsbuch.Databases
{
    ...
    public class SQL : SQLBase
    {
        ...
        {
            using (SqlConnection con = new SqlConnection(GetConnectionString(host, username, password, database)))
            {
                await con.OpenAsync();
                con.Close();
            }
        ...
        public override async Task UpdateAsync()
        {
            ...
            if (Version < (checkVersion = new Version(1, 0, 8, 0)))
            {
                await UpdateAsync(checkVersion);
                Version = checkVersion;
            }
        ...
        public override async Task CreateDataBaseAsync(string host, string user, string password, string database)
        {
            ...
            using (var command = new SqlCommand(sb.ToString(), connect))
                await command.ExecuteNonQueryAsync();
            sb.Clear();
            sb.AppendLine("INSERT INTO version VALUES ('1.0.8.0');");
            using (var command = new SqlCommand(sb.ToString(), connect))
                await command.ExecuteNonQueryAsync();
            sb.Clear();
            await CreateTriggerAsync();
            sb.AppendLine("CREATE VIEW bank_total_amount AS Select COALESCE(SUM(amount),0) amount from bank");
            using (var command = new SqlCommand(sb.ToString(), connect))
                await command.ExecuteNonQueryAsync();
            sb.Clear();
            sb.AppendLine("CREATE VIEW barge_total_amount AS Select COALESCE(SUM(amount),0) amount from barge;");
            using (var command = new SqlCommand(sb.ToString(), connect))
                await command.ExecuteNonQueryAsync();
            sb.Clear();
        ...
        protected override void CreateFixedTables(StringBuilder sb)
        {
            ...
            sb.AppendLine("[assistants] image,");
        ...
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("CREATE TRIGGER books_AUPD ON books AFTER UPDATE As");
            sb.AppendLine("BEGIN ");
            sb.AppendLine("Update clients SET amount=account_transfer + (Select COALESCE(sum(amount),0) from books where clients.id=books.id);");
            sb.AppendLine("Update clients SET lastbook=(Select max(date) from books where clients.id=books.id);");
            sb.AppendLine("END");
            using (var command = new SqlCommand(sb.ToString(), connect))
                await command.ExecuteNonQueryAsync();
            sb.Clear();
            sb.AppendLine("CREATE TRIGGER books_AINS ON books AFTER INSERT As");
            sb.AppendLine("BEGIN ");
            sb.AppendLine("Update clients SET amount=account_transfer +(Select COALESCE(sum(amount),0) from books where clients.id=books.id);");
            sb.AppendLine("Update clients SET lastbook=(Select max(date) from books where clients.id=books.id);");
            sb.AppendLine("END");
            using (var command = new SqlCommand(sb.ToString(), connect))
                await command.ExecuteNonQueryAsync();
            sb.Clear();
            sb.AppendLine("CREATE TRIGGER books_ADEL ON books AFTER DELETE As");
            sb.AppendLine("BEGIN ");
            sb.AppendLine("Update clients SET amount=account_transfer +(Select COALESCE(sum(amount),0) from books where clients.id=books.id);");
            sb.AppendLine("Update clients SET lastbook=(Select max(date) from books where clients.id=books.id);");
            sb.AppendLine("END");
        ...
        public override async Task RestoreAsync(string filename)
        {
            ...
            sb.AppendLine("DROP TABLE assistants;");
            sb.AppendLine("DROP TABLE bank;");
            sb.AppendLine("DROP TABLE barge;");
            sb.AppendLine("DROP TABLE hard_cash;");
            sb.AppendLine("DROP TABLE books;");
