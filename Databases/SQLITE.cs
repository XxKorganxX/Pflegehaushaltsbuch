    ...
    public class SQLITE : SQLBase
    {
        ...
        public override async Task CreateDataBaseAsync(string host, string username, string password, string database)
        {
            try
            {
                ...
                sb.AppendLine("INSERT INTO version VALUES (0, '1.0.9.0');");
        ...
        public override async Task UpdateAsync(Version version)
        {
            ...
            if (version == new Version("1.0.9.0"))
            {
                ...
                using (SQLiteCommand renameCommand = new SQLiteCommand("SELECT COUNT(*) FROM pragma_table_info('layouts') WHERE name='assistants';", connection))
                {
                    long existingAssistantsLayout = (long)await renameCommand.ExecuteScalarAsync();
                    renameCommand.CommandText = "SELECT COUNT(*) FROM pragma_table_info('layouts') WHERE name='employees';";
                    long existingEmployeesLayout = (long)await renameCommand.ExecuteScalarAsync();
                    if (existingAssistantsLayout > 0 && existingEmployeesLayout == 0)
                    {
                        renameCommand.CommandText = "ALTER TABLE layouts RENAME COLUMN assistants TO employees;";
                        await renameCommand.ExecuteNonQueryAsync();
                    }
                }
